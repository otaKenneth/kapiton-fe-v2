...
export async function productRelated (product_id) {
    const res = await fetch(api_address + `product/related/${product_id}`);
    if (!res.ok) throw new Error("Failed to fetch product related.");
    return res.json();
}

export async function productReviews (product_id) {
    const res = await fetch(api_address + `product/reviews/${product_id}`);
...
export async function selectDeliveryAddress(token = '', data) {
    const res = await fetch(api_address + `cart/delivery-computation`, {
        method: "POST",
        headers: {
            "Content-Type": "application/json",
            "Authorization": `Bearer ${token}`
