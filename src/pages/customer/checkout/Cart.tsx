        ...
        <div className="bg-white shadow rounded-lg p-4 flex-[2]">
          <ul>
            {cartItems.map((item) => (
              <li
                key={item.id}
                className="grid grid-cols-6 items-center border-b py-4 last:border-b-0 w-full lg:w-[40vw] gap-2"
              >
                {/* Product Image */}
                ...
                <div className="col-span-3 flex flex-col">
                  ...
                  {formatPeso(item.product.product_price)} x {item.quantity}
                  </div>
                </div>
                {/* Quantity Controls */}
          ...
          <div className="bg-white shadow rounded-lg p-4">
            ...
            <div className="text-gray-700">
              {state.user?.defaultAddress ? (
                <>
                  <div>{state.user.defaultAddress.name}</div>
                  <div>{state.user.defaultAddress.street}</div>
                  <div>
                    {state.user.defaultAddress.city},{" "}
                    {state.user.defaultAddress.province}
                  </div>
                  <div>{state.user.defaultAddress.zip}</div>
                  <div>{state.user.defaultAddress.phone}</div>
                </>
              ) : (
